...
export function Copilot() {
  ...
  return (
    <>
      {/* Floating launcher — clearly visible, inset from the corner */}
      <motion.button
        initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        transition={{ delay: 0.5, type: 'spring', stiffness: 380, damping: 22 }}
        whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.94 }}
        onClick={() => setOpen((o) => !o)}
        className="fixed bottom-6 right-6 z-[60] flex h-16 w-16 items-center justify-center rounded-full bg-cat-yellow text-ink-900 shadow-glow ring-2 ring-cat-yellow/40"
        aria-label="Open CAT Copilot"
      >
        <AnimatePresence mode="wait">
          {open ? (
            <motion.span key="x" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }}><X className="h-7 w-7" strokeWidth={2.5} /></motion.span>
          ) : (
            <motion.span key="b" initial={{ rotate: 90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }}><Bot className="h-7 w-7" strokeWidth={2.2} /></motion.span>
          )}
        </AnimatePresence>
        {!open && <span className="absolute -right-0.5 -top-0.5 flex h-3.5 w-3.5"><span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-ok opacity-75"></span><span className="relative inline-flex h-3.5 w-3.5 rounded-full bg-ok ring-2 ring-ink-900"></span></span>}
      </motion.button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-28 right-6 z-[60] flex h-[34rem] w-[23rem] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border border-white/[0.08] bg-ink-700/95 shadow-card backdrop-blur-xl"
          >
            <div className="flex items-center gap-3 border-b border-white/[0.06] px-4 py-3.5">
              <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-cat-yellow/15 text-cat-yellow"><Sparkles className="h-5 w-5" /></div>
              <div>
              ...
              {messages.map((m, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className={`max-w-[86%] whitespace-pre-line rounded-2xl px-3.5 py-2.5 text-[13px] leading-relaxed ${
                    m.role === 'user' ? 'rounded-br-md bg-cat-yellow text-ink-900' : 'rounded-bl-md bg-ink-500/60 text-ink-50'}`}>
                    {m.text}
                  </div>
                </motion.div>
              ))}
              ...
              <div className="scrollbar-thin flex gap-1.5 overflow-x-auto pb-1">
                {PROMPTS.map((p) => (
                  <button key={p} onClick={() => send(p)} disabled={typing}
                    className="group flex shrink-0 items-center gap-1 whitespace-nowrap rounded-full border border-white/10 bg-ink-600/50 px-3 py-1.5 text-[11px] text-ink-100 transition-colors hover:border-cat-yellow/30 hover:bg-ink-500/50 disabled:opacity-40">
                    {p}<ArrowUpRight className="h-3 w-3 text-ink-200 transition-colors group-hover:text-cat-yellow" />
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="border-t border-white/[0.06] p-3">
              <form onSubmit={(e) => { e.preventDefault(); send(input); }} className="flex items-center gap-2">
                ...
                <button type="submit" className="flex h-9 w-9 items-center justify-center rounded-lg bg-cat-yellow text-ink-900 transition-colors hover:bg-cat-soft"><Send className="h-4 w-4" /></button>
              </form>
            </div>
          </motion.div>
        )}
